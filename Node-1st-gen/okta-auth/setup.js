...
const EXAMPLE_ENDPOINT = DEPLOYING
  ? `
...
(async () => {
  ...
  console.log(
    DEPLOYING
      ? "Checking configuration for deployment..."
...
`;
  ...
  if (DEPLOYING) {
    try {
      ...
      if (
        ...
        console.log(
          "Cloud Functions environment not configured. " + "Configuring...",
        );
        oktaOrgUrl = oktaOrgUrl || (await ask(OKTA_URL_PROMPT));
        ...
        console.log(
          "Cloud Functions environment configuration found. " +
            "Skipping setup.",
        );
      }
    } catch {
      console.error(
        "Couldn't set up the Cloud Functions environment. " +
  ...
  try {
    ...
    const contents = `GOOGLE_APPLICATION_CREDENTIALS="${serviceAccountFile}"\n`;
