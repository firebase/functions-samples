...
Demo.prototype.displayAllUsers = function () {
  ...
  usersRef.on(
    ...
    function (snapshot) {
      ...
      var materialSwitchContainer =
        userElement.getElementsByClassName("mdl-switch")[0];
      if (componentHandler) {
      ...
      var notificationEnabledElement = userElement.getElementsByClassName(
        "demo-notifications-enabled",
      )[0];
      var notificationsEnabledRef = snapshot.ref.child("notificationTokens");
      notificationsEnabledRef.on(
        "value",
        function (notificationsEnabledSnapshot) {
          notificationEnabledElement.style.display =
            notificationsEnabledSnapshot.hasChildren() ? "inline" : "none";
        },
      );
      ...
      var followUserRef = firebase
        .database()
        .ref("followers/" + uid + "/" + this.currentUid);
